...
const Appartments = ({ villageId }) => {
    ...
    const filterOptions = useMemo(() => [
        {
            ...
            label: t("Village"),
            options: [
                { value: "all", label: t("All Villages") },
    ...
    if (activeVillageId && activeVillageId !== "undefined" && activeVillageId !== "null") {
        queryParams.append("village_id", activeVillageId);
    }

    const { refetch: refetchAppartment, loading: loadingAppartmentGet, data: AppartmentData } = useGet({
    ...
    useEffect(() => {
        const appartmentList = paginationInfo?.data || (Array.isArray(paginationInfo) ? paginationInfo : []);
    ...
    const handleEdit = (Appartment) => {
        ...
        setRowEdit({
            ...
            type: fullAppartmentData?.type?.id?.toString() || fullAppartmentData?.appartment_type_id?.toString() || "",
            appartment_type_id: fullAppartmentData?.type?.id?.toString() || fullAppartmentData?.appartment_type_id?.toString() || "",
    ...
    const handleSave = async () => {
        ...
        const result = await changeState(url, t("Appartmentupdatedsuccessfully!"), body);
    ...
    const columns = [
        {
            ...
            label: t("Unit"),
            render: (row) => (
                <Link to={`/units/details/${row.id}`} className="text-blue-600 hover:underline font-semibold">
                    {row.name || "—"}
                </Link>
            ),
        },
            ...
            <Dialog open={isBansDialogOpen} onOpenChange={setIsBansDialogOpen}>
                <DialogContent className="sm:max-w-[460px] rounded-2xl !p-6 bg-white">
                    ...
                    <div className="flex justify-end border-t !pt-4 mt-2">
                        ...
                        >
                            {loadingPut ? t("Saving...") : t("Save Changes")}
                        </button>
                    </div>
                </DialogContent>
            </Dialog>
