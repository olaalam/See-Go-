...
export default function Addvillage_roles() {
  ...
  const { id: villageId, adminId } = useParams();
  ...
  useEffect(() => {
    const fetchRoles = async () => {
      ...
      try {
        ...
        toast.error("An error occurred while fetching roles!", { position: "top-right", autoClose: 3000 });
      } finally {
        dispatch(hideLoader());
      }
    };
  ...
  const handleSubmit = async () => {
    ...
    body.append("roles", formData.roles.join(','));
    ...
    try {
      ...
      if (response.ok) {
        toast.success("Village role added successfully!", { position: "top-right", autoClose: 3000 });
        setFormData({
          name: "",
          roles: [], // Reset to empty array after successful submission
          status: "",
        });
        navigate(`/villages/single-page-v/${villageId}/admin/${adminId}/`);
      } else {
  ...
  const fields = [
    ...
    },
    {
      ...
      multiple: true, // Crucial: This tells the Add component to enable multi-select
    },
    {
      type: "select",
      placeholder: "Status",
