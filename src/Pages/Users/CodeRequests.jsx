...
const CodeRequests = () => {
  ...
  const hasPermission = (permission) => {
    const match = permission.match(/^CodeRequests(.*)$/i);
  ...
  const fetchCodeRequests = async () => {
    dispatch(showLoader());
    try {
      const response = await fetch("https://bcknd.sea-go.org/admin/code_request", {
        method: "GET",
        headers: {
      ...
      const rawList = result?.code_requests?.data || result?.data || [];
  ...
  const handleStatusSubmit = async () => {
    ...
    try {
      const response = await fetch(
        `https://bcknd.sea-go.org/admin/code_request/status/${selectedRow.id}?status=${currentStatusAction}`,
        {
          method: "PUT",
            ...
            <div className="flex items-start gap-3">
              ...
              <div>
                ...
                <p className="text-sm text-gray-500 !mt-2">
                  Are you sure you want to <span className="font-semibold">{currentStatusAction}</span> the code request for{" "}
